...
export class UsersData{
    ...
    public displayUserDetails(){
        ...
        console.log(columnWidths.map((width, index) => headers[index].padEnd(width)).join(' | '));
    ...
    public sortByName(usersData:any, descOrder:number = 0){
        usersData.sort((a:any, b:any) => {
            if(descOrder == 1){
                ...
                return nameComparison !== 0 ? nameComparison : b.rollNumber - a.rollNumber;
            }
    ...
    public deleteUser(rollNumber:number){
        const indexToRemove = this.usersData.findIndex((user:any) => user.rollNumber === rollNumber);
        if (indexToRemove !== -1) {
