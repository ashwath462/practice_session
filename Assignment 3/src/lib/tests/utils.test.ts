import { isValidCredInput } from "../../utils/utils";
...
test("email Regex checker", () => {
  let testcases = [
    "ashwath@gmail.com",
    "tuhina@gmail.com",
    "20bit109@ietdavv.edu.in",
    "ashwath.arora@gonuclei.in",
  ];
...
test("credentials valid", () => {
  ...
  testcases.forEach((testcase) => {
    if (testcase.length === 3)
      expect(isValidCredInput(testcase[0], testcase[1], testcase[2])).toBe(
        true,
      );
...
test("credentials invalid", () => {
  ...
  testcases.forEach((testcase) => {
    if (testcase.length === 3)
      expect(() =>
        isValidCredInput(testcase[0], testcase[1], testcase[2]),
      ).toThrow();
