<script lang="ts">
	...
	let city = '',
		searchListItems: CityDetails[] = [],
		popularCities: CityDetails[] = [],
		recentSearchedCities: CityDetails[] = [];
	let error: string = '';
...
<div class="flex flex-col w-full">
	<div class="join p-2">
		<input
			...
			class="input join-item w-full text-sm"
		/>
		{#if city.trim() === ''}
		...
		{:else}
			...
			<div
				...
				class="btn join-item bg-white border-none"
			>
				<Close />
			</div>
		{/if}
	...
	<div class="flex flex-col">
		{#if searchListItems.length === 0}
			{#if recentSearchedCities.length >= 1}
				<SearchList {type} title="Recent Searches" cities={recentSearchedCities} bind:error />
			{/if}
			<SearchList {type} title="Popular cities" cities={popularCities} bind:error />
		{:else}
			<SearchList {type} title="Search cities" cities={searchListItems} bind:error />
		{/if}
	</div>
</div>
